...
tmp <- ipwpoint(
  ...
  family      = "binomial",
  link        = "logit",
  denominator = ~ age + meno + size + factor(grade) + nodes + prog + estrg,
  data        = df
)
...
obj_cox_w_A1 <- coxph(Surv(time, status) ~ age + meno + size + grade + nodes + prog + estrg,
                     data = df, subset = (A == 1)) # for A = 1
obj_cox_w_A0 <- coxph(Surv(time, status) ~ age + meno + size + grade + nodes + prog + estrg,
...
tbl <- tbl_merge(
  ...
  modify_caption("Table: Comparison of Cox Model Estimates for HAART Data")
