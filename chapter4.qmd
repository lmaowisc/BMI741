...
obj <- coxph(
  Surv(time, status) ~ hormone + meno + age + size + grade + nodes 
                        + prog + estrg, data = df)
...
summary(obj)
#> Call:
#> coxph(formula = Surv(time, status) ~ hormone + meno + age + size + 
...
#> hormone2    0.7073     1.4138    0.5492    0.9109
#> meno2       1.2949     0.7723    0.9038    1.8553
...
#> prog        0.8011     1.2482    0.7160    0.8964
#> estrg       1.0199     0.9805    0.9338    1.1140
#> 
...
# Tidy regression results as a data frame
library(broom)
...
plot(
  ...
  ylab = "Martingale residuals",
  main = "Number of positive lymph nodes", 
    cex.main = 1.5,
  cex.lab = 1.5, 
  cex.axis= 1.5
)
...
# Log-transform number of positive lymph nodes and re-plot
plot(
  log(df$nodes), mart_resid,
  xlab = "Log(Number of positive lymph nodes)", 
  ylab = "Martingale residuals",
  main = "Log(Number of positive lymph nodes)", 
    cex.main = 1.5,
  cex.lab = 1.5, 
  cex.axis= 1.5
)
...
## GBC Cox model with time-varying hormone effect (stratified by tumor grade)
df$hormone <- as.numeric(df$hormone)
...
summary(obj_horm_tv)
#>                  coef exp(coef)  se(coef)      z Pr(>|z|)    
#> hormone     -0.459776  0.631425  0.245364 -1.874   0.0610 .  
#> tt(hormone)  0.002093  1.002095  0.007549  0.277   0.7816    
#> meno2        0.248076  1.281557  0.156303  1.587   0.1125    
#> agec2       -0.498373  0.607518  0.197753 -2.520   0.0117 *  
#> agec3       -0.494745  0.609726  0.249692 -1.981   0.0475 *  
#> size         0.006650  1.006672  0.003955  1.682   0.0927 .  
#> log_nodes    0.477858  1.612616  0.066183  7.220 5.19e-13 ***
#> prog        -0.229851  0.794652  0.057239 -4.016 5.93e-05 ***
#> estrg        0.030966  1.031451  0.046585  0.665   0.5062    
#> ---
#> Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# Get c(beta1, beta2) and 2 x 2 variance matrix
