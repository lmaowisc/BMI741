...
To show such information, additional plot features are needed.
...
chan_fig <- channing_sub |> 
  ...
  geom_vline(xintercept = 60, linewidth = 1) + # reference line at age 60
...
For each subject, we use a gray line to represent the follow-up period 
during which the event does not occur. The check-up times can be plotted
on top as dots if such data are available.
The interval $[L_i, R_i]$
($R_i<\infty$) that contains the event is distinguished from the follow-up period 
by a black line segment.
