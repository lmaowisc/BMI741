...
fig_rats_sub_line <- rats_sub |> 
  ...
  # Horizontal line with right arrow at end
  geom_segment(x = 0, xend = 70, y = 1, yend = 1,
               arrow = arrow(length = unit(0.2, "cm"), ends = "last", type = "open"),
               size = 0.5) +
  # Points for events and censored observations
  # c(23, 19) for censoring and event 
  ...
  scale_x_continuous(limits= c(0, max(rats_sub$time)), breaks = c(0, unique(rats_sub$time)), 
...
# stack the two plots
# collect legend from one plot only
library(patchwork)
fig3_2 <-  fig_rats_sub_swim / fig_rats_sub_line +
  plot_layout(heights = c(0.8, 0.2))


fig3_2

# ggsave("images/km_rats.png", fig3_2, width = 8, height = 4)
# ggsave("images/km_rats.eps", fig3_2, width = 8, height = 4)
...
#------------------------------------------------------------------------------
# 5a. Plot the estimated cumulative hazard function
#------------------------------------------------------------------------------
par(mfrow = c(1, 1))
plot(
  stepfun(ts, c(0, L)), 
...
#------------------------------------------------------------------------------
# 5b. Plot the estimated cumulative hazard function - ggplot version
#------------------------------------------------------------------------------
library(ggplot2)
# extend last value to create step function
df_NA <- data.frame(
  time   = c(0, ts),
  cumhaz = c(0, L)
) |> 
...
# ggsave("images/km_rat_NA.png", width = 8, height = 3.5)
# ggsave("images/km_rat_NA.eps", device = cairo_ps, width = 8, height = 3.5)
...
#>  1    23    100       0        1    1        0              1    1    
#>  2    34     99       1        0    0.990    0.0102         1    0.970
#>  3    39     98       1        0    0.980    0.0144         1    0.952
#>  ...

# Save Kaplan--Meier curve to EPS using base R
# postscript("images/km_rat_KM.eps",
#            width = 8, height = 5,
...
# Summaries including 90% and 80% percentiles
