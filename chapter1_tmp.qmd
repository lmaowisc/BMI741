...
We can now feed this dataset to standard functions for survival analysis,
e.g., `survfit()` for the Kaplan--Meier estimator:
...
`reframe()`. But first we use `survfit()` to get the Kaplan--Meier estimates:
...
Then the event-imputation and complete-case estimates
(with `ecdf()` for empirical distribution function):
...
death_rates <- df |> 
  group_by(hormone) |> 
  summarize(
    death_rate = as.character(round(event_rate(time, status) * 12, 3))
  ) |> 
  pivot_wider(
    names_from = hormone,
...
ce_rates <- df_ce |> 
  group_by(hormone) |> 
  summarize(
    ce_rate = as.character(round(event_rate(time, status > 0) * 12, 3))
  ) |> 
  pivot_wider(
    names_from = hormone,
    values_from = ce_rate
