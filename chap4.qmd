...
-   **Summary results**
    ...
    -   Hormone-treated women 70.7% times as likely to relapse/die as untreated
...
![](images/cox_gbc_nodes.png){fig-align="center" width="80%"}
:::
...
::: fragment
-   **Transformations**
    -   Categorize age: `agec=1`: $\leq 40$ yrs; `agec=2`: $(40, 60]$ yrs; `agec=3`: $> 60$ yrs
    -   Log-transform nodes: `log_nodes = log(nodes)`
...
::: fragment
-   **Categorize age**
    -   Age differences masked by inappropriate linear form ($p$-value 0.309)

![](images/cox_nonlinear_age.png){fig-align="center" width="70%"}
:::
...
## Example: German Breast Cancer (XIX)

::: fragment
-   **Final model**

![](images/cox_gbc_final.png){fig-align="center" width="75%"}
:::
...
-   **Time-varying coefficients**
    ...
    #| eval: false
    #| echo: true
# Fit a Cox model with time-varying hormone treatment effect
obj_horm_tv <- coxph(
  Surv(time, status) ~ hormone + tt(hormone) + meno + agec + size
                     + log_nodes + prog + estrg + strata(grade),
  data = df,
  # Define time transformation: x * t (linear time interaction)
  tt = function(x, t, ...) x * t
)
    ```
:::
...
-   Problem 4.8
-   Problem 4.15
-   Problems 4.17 and 4.23
-   Extra credit: Problem 4.24
