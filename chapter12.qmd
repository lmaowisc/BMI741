...
library(broom)     # tidy() for survfit output
library(grid)      # unit()

# -----------------------------------------------------------------------------
# 2. Read GBC multi-state data
# -----------------------------------------------------------------------------
gbc_ms <- read.table("Data//German Breast Cancer Study//gbc_ms.txt")

# -----------------------------------------------------------------------------
# 3. Create a transition label
# -----------------------------------------------------------------------------
# Transition labels:
#   1: Remission -> Relapse
#   2: Remission -> Death
...
# Rescale hormone receptor measurements
...
# -----------------------------------------------------------------------------
# 1. Define the 3-state transition matrix
# -----------------------------------------------------------------------------
tmat <- transMat(
  list(c(2, 3), c(3), c()),
...
# -----------------------------------------------------------------------------
# 2. Fit a null Cox model stratified by transition
# -----------------------------------------------------------------------------
# This gives Nelson--Aalen-type estimates of cumulative transition intensities
cox_fit <- coxph(Surv(start, stop, status) ~ strata(trans), data = gbc_ms)
ms_obj  <- msfit(cox_fit, trans = tmat)

# -----------------------------------------------------------------------------
# 3. Summarize and plot cumulative transition intensities
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4. Helper to plot cumulative intensities by hormone subgroup
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. Optional saving of cumulative intensity figure
# -----------------------------------------------------------------------------
# ggsave("images//multistate_cumint_gbc.eps", width = 8, height = 4.5)
# ggsave("images//multistate_cumint_gbc.png", width = 8, height = 4.5)

# -----------------------------------------------------------------------------
# 6. State-occupancy probabilities
# -----------------------------------------------------------------------------
# predt = 0 means state occupation is predicted from baseline
...
# -----------------------------------------------------------------------------
# 7. Helper to plot state-occupancy probabilities by hormone subgroup
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 8. Four-state version
# -----------------------------------------------------------------------------
# Distinguish death in remission from death after relapse
tmat4 <- transMat(
  list(c(2, 3), c(4), c(), c()),
...
# -----------------------------------------------------------------------------
# 9. Optional saving of occupancy figure
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 1. Transition 0 -> 1: remission to relapse
# -----------------------------------------------------------------------------
obj01 <- coxph(
  Surv(start, stop, status) ~ hormone + meno + agec + size + nodes + prog + estrg +
    strata(grade),
  data   = gbc_ms,
  subset = (from == 0 & to == 1)
)
...
# -----------------------------------------------------------------------------
# 2. Transition 0 -> 2: remission to death
# -----------------------------------------------------------------------------
obj02 <- coxph(
  Surv(start, stop, status) ~ hormone + meno + size + nodes + prog + estrg +
    strata(grade),
  data   = gbc_ms,
  subset = (from == 0 & to == 2)
)
...
# -----------------------------------------------------------------------------
# 3. Transition 1 -> 2: relapse to death
# -----------------------------------------------------------------------------
# tt(start) is used to represent time spent in the current state
obj12 <- coxph(
  Surv(start, stop, status) ~ hormone + meno + agec + size + nodes + prog + estrg +
...
# =============================================================================
# (D) Tabulate Model Results
# =============================================================================

# -----------------------------------------------------------------------------
# 1. Helper to extract HRs, 95% CIs, and p-values
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. Results by transition
# -----------------------------------------------------------------------------
extract_results(obj01) # 0 -> 1
extract_results(obj02) # 0 -> 2
extract_results(obj12) # 1 -> 2

# =============================================================================
# (E) Dynamic Prediction via the Multi-State Model
# =============================================================================

# -----------------------------------------------------------------------------
# 1. Minimal example of expand.covs()
# -----------------------------------------------------------------------------
test_df <- gbc_ms |>
  select(1:9) |>
...
# -----------------------------------------------------------------------------
# 2. Cox model with transition-specific expanded covariates
# -----------------------------------------------------------------------------
# expand.covs() creates transition-specific versions of the covariates
gbc_ms1 <- expand.covs(
  gbc_ms |> rename(failcode = trans),
  covs = c("hormone", "meno", "agec", "size", "nodes", "prog", "estrg", "grade")
)
...
# -----------------------------------------------------------------------------
# 3. New subjects for prediction from month 12
# -----------------------------------------------------------------------------
# Profile:
#   pre-menopausal, age < 40, size = 25 mm, grade = 2, nodes = 2,
...
# -----------------------------------------------------------------------------
# 4. Predict state-occupancy probabilities from month 12
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. Helper to plot predicted occupancy probabilities
# -----------------------------------------------------------------------------
...
# Start in remission at month 12
...
# Start post-relapse at month 12
...
# -----------------------------------------------------------------------------
# 6. Optional saving of predicted occupancy figure
# -----------------------------------------------------------------------------
# ggsave("images//multistate_pred_gbc.eps", width = 8, height = 7)
# ggsave("images//multistate_pred_gbc.png", width = 8, height = 7)

# =============================================================================
# (F) Landmarking at a Single Landmark Time
# =============================================================================

# -----------------------------------------------------------------------------
# 1. Flatten to subject level
# -----------------------------------------------------------------------------
gbc_fl <- gbc_ms |>
  group_by(id) |>
  summarize(
    surv_time = max(stop),
    death     = any(to == 2 & status == 1),
    rel       = any(from == 1),
    rel_time  = ifelse(rel, min(start[from == 1]), Inf),
...
# -----------------------------------------------------------------------------
# 3. Cox model for death in the landmarked data
# -----------------------------------------------------------------------------
cox_s0_fit <- coxph(
  Surv(surv_time, death) ~ strata(rel_s0) + hormone + meno + agec +
    size + nodes + prog + estrg + grade,
  data = gbc_lm_s0
)
...
# -----------------------------------------------------------------------------
# 4. New data for landmark predictions
# -----------------------------------------------------------------------------
...
nd_lm_h1_rel <- nd_lm_h1_norel |>
  mutate(rel_s0 = 1)

nd_lm_h2_norel <- nd_lm_h1_norel |>
  mutate(hormone = factor(2, levels = c("1", "2")))

nd_lm_h2_rel <- nd_lm_h1_rel |>
  mutate(hormone = factor(2, levels = c("1", "2")))

# -----------------------------------------------------------------------------
# 5. Survival predictions from the landmark time onward
# -----------------------------------------------------------------------------
surv_lm_h1_norel <- survfit(cox_s0_fit, newdata = nd_lm_h1_norel) |>
  tidy() |>
  mutate(trt = "No Hormone", rel = "No Relapse")

surv_lm_h2_norel <- survfit(cox_s0_fit, newdata = nd_lm_h2_norel) |>
  tidy() |>
  mutate(trt = "Hormone", rel = "No Relapse")

surv_lm_h1_rel <- survfit(cox_s0_fit, newdata = nd_lm_h1_rel) |>
  tidy() |>
  mutate(trt = "No Hormone", rel = "Post-Relapse")

surv_lm_h2_rel <- survfit(cox_s0_fit, newdata = nd_lm_h2_rel) |>
  tidy() |>
...
# Plot landmark-based survival predictions
surv_lm |>
  ggplot(aes(x = time, y = estimate, color = trt)) +
  geom_step(linewidth = 1) +
  geom_ribbon(
    aes(ymin = conf.low, ymax = conf.high, fill = trt),
    alpha = 0.2,
    color = NA
  ) +
...
# -----------------------------------------------------------------------------
# 6. Save landmark prediction figure
# -----------------------------------------------------------------------------
...
# =============================================================================
# (G) Landmarking via a Supermodel
# =============================================================================

# -----------------------------------------------------------------------------
# 1. Create stacked landmark datasets at 3, 6, ..., 36 months
# -----------------------------------------------------------------------------
...
for (s0 in lm_times) {
  gbc_lm_s <- gbc_fl |>
    filter(surv_time >= s0) |>
    mutate(
      rel_s     = as.numeric(rel_time < s0),
      rel_dur_s = pmax(s0 - rel_time, 0),
      s         = s0
    )
...
# -----------------------------------------------------------------------------
# 2. Fit the landmark supermodel
# -----------------------------------------------------------------------------
# rel_s is the relapse status at landmark time s
# strata(s) allows a different baseline hazard by landmark time
# cluster(id) accounts for repeated inclusion of the same subject across landmarks
cox_super_fit <- coxph(
  Surv(surv_time, death) ~
    (rel_s + hormone + meno + agec + size + nodes + prog + estrg + grade) * s +
    strata(s) + cluster(id),
  data = gbc_lm_super
)

# -----------------------------------------------------------------------------
# 3. Optional prediction from the supermodel
# -----------------------------------------------------------------------------
# Prediction from the landmark supermodel can be constructed as an extension
# of the single-landmark workflow above
