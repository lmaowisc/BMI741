...
To convert this vector to the tidy format, use `parse_number()` to parse out the number and `str_detect()` to detect whether the entry contains "+":

```{r}

df <- tibble(
  MP = MP,
  time = parse_number(MP),
  status = 1 - str_detect(MP, "\\+") # escape literal "+"
)

df
```

#### Facet plotting Figure 1.2
