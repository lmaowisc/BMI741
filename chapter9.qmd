...
coeff_AG <- summary(obj.AG)$coeff
coeff_frail <- summary(obj.frail)$coeff
coeff_pm <- summary(obj.pm)$coeff
...
library(pscl)
# Fit ZIP model 
zip_model <- zeroinfl(count ~ treat + age + sex 
                              + offset(log(follow_up)) # Poisson model
                            | treat + age+ sex, # Logistic model
                        data = df)
