...
BSfun <- function(timepoints, time, status, St, t) {
  ...
  eps <- max(1e-8, 1e-8 * max(c(timepoints, time, t), na.rm = TRUE))
...
predsurv_cox <- function(fit, newx) {
  # fit: coxph object
  # newx: design matrix (n x p)

  # Extract coefficients
  beta <- coef(fit)

  # Linear predictors
  eta <- as.vector(newx %*% beta)
  ...
  # Compute survival: S(t|z) = exp{-Lambda0(t) * exp(eta)}
  St <- outer(exp(eta), Lambda0, function(e, L) exp(-L * e))
